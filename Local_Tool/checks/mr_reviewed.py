...
def count_all_time_reviewed_prs():
    """Count the number of reviewed PRs in the project."""
    ...
    for pr in prs:
        ...
        if review_response.status_code != 200:
            print(f"Error loading reviews for PR {pr_number}: {review_response.status_code}")
...
def load_scored_prs():
    """Load the number of scored PRs from the API."""
    ...
    if response.status_code == 200:
        ...
        print(f"Error loading scored PRs: {response.status_code}")

        return None


def update_score_for_reviews():
    """Update the score for the number of reviewed PRs in the project."""
